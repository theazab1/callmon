...
class CallMonitorService : Service() {
    ...
    private fun startRecording() {
        if (recorder != null) {
            // تسجيل شغال بالفعل، نتجاهل الطلب عشان ما يحصلش تضارب (مثال: رنة ثم رد)
            return
        }
        ...
        recorder = MediaRecorder().apply {
            try {
                // تحديث: لتخطي الحظر الصامت في أندرويد 10+، نستخدم VOICE_RECOGNITION
                // لأنه الخيار الوحيد الذي قد يتجاوز حظر الميكروفون أثناء المكالمات على بعض الأجهزة.
    ...
    private fun showReminderOverlay() {
        ...
        // مسح أي رسالة تذكير سابقة عشان ما تتراكمش الرسائل فوق بعض
        // في حالة استلام أحداث متتالية (مثل RINGING ثم OFFHOOK)
        hideReminderOverlay()
